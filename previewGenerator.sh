# ----------------------------------------------
export folder=D:\Movies
export vframes=10
export width=384
export height=216
    rem w = h*16/9
export filetypes=*.mp4
# ----------------------------------------------
...
for /f "usebackq delims=" %%f in (`dir /b %filetypes%`) do (
    if not exist "preview\%%~nf.mp4" (  
        for /f $i in ('ffprobe -v error -show_entries format^=duration "%%f" -of default^=noprint_wrappers^=1:nokey^=1') do export length=$i
        export /a length=!length!+0
        export /a fps=!length!/%vframes%
